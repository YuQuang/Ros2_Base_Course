...
def control_by_direction(direction: str, ser: Serial, timer: Timer) -> None:
    if direction == "left": ser.write(b"left\n")
    elif direction == "right": ser.write(b"right\n")
    elif direction == "forward": ser.write(b"forward\n")
    elif direction == "back": ser.write(b"back\n")
    else:  ser.write(b"stop\n")
    if not timer.isAlive: timer.start()


def calc_fingers(landmarks):
    count = 0
    sight = False
    direction = "None"
    landmark = landmarks.landmark

    if (landmark[0].x - landmark[1].x) > 0: sight = True  # 正反面
    if (landmark[6].y - landmark[8].y) > 0: count+=1      # 計算食指
    if (landmark[10].y - landmark[12].y) > 0: count+=1    # 計算中指
    if (landmark[14].y - landmark[16].y) > 0: count+=1    # 計算無名指
    if (landmark[18].y - landmark[20].y) > 0: count+=1    # 計算小拇指

    if sight:
        if (landmark[2].x - landmark[4].x) > 0: count+=1      # 計算食指
    else:
        if (landmark[4].x - landmark[2].x) > 0: count+=1      # 計算食指
    
    if abs(landmark[0].x - landmark[4].x) > abs(landmark[0].y - landmark[4].y):
        if (landmark[0].x - landmark[4].x) > 0: direction = "left"
        else: direction = "right"
    else:
        if (landmark[0].y - landmark[4].y) > 0: direction = "forward"
        else: direction = "back"
    
    if count == 5: direction = "stop"
...
def main():
    ...
    # Arduino 準備 ############################################################
    ser = serial.Serial("/dev/ttyACM0", 115200, timeout=0.5) #使 用樹莓派的GPIO口連線序列口
    timer = Timer(1.0, ser.write, b"stop\n")
        ...
        while True:
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                    ...
                    # 計算舉起來的手指數量
                    finger_number, direction = calc_fingers(hand_landmarks)
                    # 依照判斷的方向控制
                    control_by_direction(direction, ser, timer)
